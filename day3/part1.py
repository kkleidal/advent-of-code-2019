...
def intersect1d(range1, range2):
    ...
    #    return range(min1, min(max1, max2)
...
def problem(inputs):
    ...
    for segment1 in path1.line_segments():
        for segment2 in path2.line_segments():
            # print(segment1, segment2, list(segment1 & segment2))
            intersections.update(segment1 & segment2)
    intersections.remove(Point(0, 0))
    return min(point.distance for point in intersections)
