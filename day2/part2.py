...
def dovetail(N):
    for summ in range(0, N + N + 1):
        for noun in range(max(0, summ - N), min(N, summ) + 1):
            verb = summ - noun
            yield noun, verb
...
for i in range(20):
    dt = sorted(dovetail(i))
    ex = sorted(exhaustive(i))
    assert tuple(dt) == tuple(ex), "%s != %s" % (dt, ex)

def part2(upper_bound=99):
    ...
    for noun, verb in dovetail(upper_bound):
        current_run = list(program)
        current_run[1] = noun
        current_run[2] = verb
        compute(current_run)
        if current_run[0] == 19690720:
